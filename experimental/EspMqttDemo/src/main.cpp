...
static esp_err_t mqtt_event_handler(esp_mqtt_event_handle_t event)
{
    ...
    case MQTT_EVENT_CONNECTED:
        ...
        ESP_LOGE(TAG, "sent unsubscribe successful, msg_id=%d", msg_id);
        isConnected = true;
        break;
...
static void wifi_init(void)
{
    ...
    ESP_LOGE(TAG, "start the WIFI SSID:[%s] password:[%s]", WIFI_SSID, "******");
...
static void mqtt_app_start(void)
{
    ...
    mqtt_cfg.uri = "mqtt://leonding.synology.me:48888";
    mqtt_cfg.event_handle = mqtt_event_handler;

    // const esp_mqtt_client_config_t mqtt_cfg = {
    //     .uri = "mqtt://leonding.synology.me:48888",
...
void setup()
{
    ...
    ESP_LOGE(TAG, "[APP] IDF version: %s", esp_get_idf_version());
...
{
    if (isConnected) {
        int msg_id = esp_mqtt_client_publish(client, "/topic/loop", "loop", 0, 0, 0);
        ESP_LOGE(TAG, "sent publish in loop, msg_id=%d", msg_id);
    }
    delay(5000);
}