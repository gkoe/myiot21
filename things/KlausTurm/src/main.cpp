...
/// MHZ
const gpio_num_t CO2_TXD = GPIO_NUM_17;
const gpio_num_t CO2_RXD = GPIO_NUM_16;
// // #define CO2_TXD (GPIO_NUM_12) // verbunden mit Rx am Mhz
// // #define CO2_RXD (GPIO_NUM_13) // verbunden mit Tx am Mhz

/// RGB Led
const gpio_num_t R = GPIO_NUM_26;
const gpio_num_t G = GPIO_NUM_18;
const gpio_num_t B = GPIO_NUM_19;
...
void app_main()
{
  ...
  else
  {
    EspStation.init();
    // long rounds = 0;
    // Logger.info("KlausTower, app_main()", "Waiting 5 seconds for connection!");
    // while (!EspStation.isStationOnline() && rounds < 5000)
    // {
    //   vTaskDelay(1 / portTICK_PERIOD_MS);
    //   Logger.info("KlausTower, app_main()", "Waiting!!!!");
    //   rounds++;
    // }
    if (EspStation.isStationOnline())
    {
      EspConfig.setNvsIntValue("startAp", 0);
      HttpServer.init();
      Logger.info("KlausTower, app_main()", "HttpServer started");
      EspTime.init();
      EspUdp.init();
    ...
    {
      Logger.info("KlausTower, app_main()", "restart as accesspoint!");
  ...
  Thing.init();
  Logger.info("KlausTower, app_main()", "Thing created");
  ...
  rgbLed = new RgbLed(R, G, B, "RgbThing", "Test");
  ...
  while (true)
  {
    ...
    // Logger.info("KlausTower, loop()", loggerMessage);
    float co2 = mhzPtr->getLastMeasurement();
    char rgbLedNew[LENGTH_STATE];
    if (co2 > co2Red)
    {
      strcpy(rgbLedNew, "100");
    ...
    if (strcmp(rgbLedNew, rgbLed->getCurrentState()) != 0)
    {
      rgbLed->setState(rgbLedNew);
      Logger.info("KlausTower, loop(), trafficLight new: ", rgbLedNew);
    }
    round++;
    if (round > 2000)
    {
      sprintf(loggerMessage, "co2: %.2f, Trafficlight current state: %s, new state: %s, limit for yellow: %d, limit for red: %d",
              co2, rgbLed->getCurrentState(), rgbLedNew, co2Yellow, co2Red);
      Logger.info("KlausTower, loop(), Round ", loggerMessage);
      round = 0;
    }
  }
}
