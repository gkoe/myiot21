...
static char DS18B20_Address[] = "1234567887654321";
    ...
    while (1)
    {
        ...
        else
        {
            ...
            for (int i = 0; i < RESCAN_INTERVAL; i++)
            {
                ...
                {
                    ds18x20_addr_t addr = addrs[i];
                    vTaskDelay(10 / portTICK_PERIOD_MS);
                    esp_err_t err = ds18x20_measure(SENSOR_GPIO, addr, false);
                    if (err != ESP_OK)
                    {
                ...
                for (int i = 0; i < sensor_count; i++)
                {
                    ...
                    float temp_c = 0.0;
                    vTaskDelay(10 / portTICK_PERIOD_MS);
                    esp_err_t err = ds18x20_read_temperature(SENSOR_GPIO, addr, &temp_c);
                    if (err != ESP_OK)
                    {
                        printf("!!!! ERROR, measure(): %d\n", err);
                    }
...
void app_main()
{
    xTaskCreate(ds18x20_test, "ds18x20_test", configMINIMAL_STACK_SIZE * 4, NULL, 1, NULL);
}
