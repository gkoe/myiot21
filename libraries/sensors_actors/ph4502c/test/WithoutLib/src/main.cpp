...
// const float CALIBRATION_DIVIDER = 1;
...
//    d ermittelt aus den Versuchen ==> 245
...
void measurePhInLoopTask(void *pvParameter)
{
  ...
  while (1)
  {
    ...
    long value = getSumOf1000MeasurementsIn40ms(&maxValue) / 1000;
    int duration = esp_timer_get_time() / 1000 - startTime;
    float ph = k * value + d;
    printf("MaxValue: %i, Duration: %i, mV: %ld, ph: %.1f\n", maxValue, duration, value, ph);
