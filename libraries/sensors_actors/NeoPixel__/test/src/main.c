...
test_neopixel()
{
	...
	int fact = 1;
    int intesityStartIndex = 0;
    int intensity = 1;
    int increment=1;
    // int sign = 1;
	while (1) {
        int intensityTime = 500 / intensity;
		usleep(1000*intensityTime);
		// //ESP_LOGE("main", "fact = %d", fact);
		for	( int j = 0 ; j < NR_LED ; j ++ )	{
            // np_set_pixel_color(&px, j, 255);
			// np_set_pixel_rgbw(&px, j , i, i, i, i);
            // int index = (j+intesityStartIndex) % NR_LED;
            // uint8_t roundedValue = intensity;
			np_set_pixel_rgbw(&px, j ,intensity , 0, 0, 0);
            // np_set_pixel_color_hsb(&px, j, 0.5F, 0.5F, 0.5F);
		}
		np_show(&px, NEOPIXEL_RMT_CHANNEL);
        // intesityStartIndex = (intesityStartIndex+1) % NR_LED;
        intensity += increment;
        // increment = intensity;
