...
void user_task_periodic(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        ccs811_set_environmental_data (sensor, 25.1, 54.2);

        // get the results and do something with them
        if (ccs811_get_results (sensor, &tvoc, &eco2, &raw_i, &raw_v))
            printf("%.3f CCS811 Sensor periodic: TVOC %d ppb, eCO2 %d ppm, raw_i %d, raw_v %d\n",
                   (double)sdk_system_get_time()*1e-3, tvoc, eco2, raw_i, raw_v);
